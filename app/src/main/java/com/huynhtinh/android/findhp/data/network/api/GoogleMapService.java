...
public interface GoogleMapService {
    // Places
    @GET("place/nearbysearch/json?key=" + Configs.PLACE_API_KEY)
    Call<PlacesResponse> fetchPlaces(@Query("location") String location,
                                   @Query("radius") int radius,
                                   @Query("type") String type);

    @GET("place/details/json?key=" + Configs.PLACE_API_KEY)
    Call<PlaceDetailResponse> fetchPlaceDetail(@Query("placeid") String placeId);
